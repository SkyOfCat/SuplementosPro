...
const EditarSnack = () => {
  ...
  useEffect(() => {
    const cargarSnack = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (datosSnack.imagen) {
            const imagenUrl = datosSnack.imagen.startsWith("http")
              ? datosSnack.imagen
              : `${API_CONFIG.BASE_URL}${datosSnack.imagen}`;
            setImagenPrevia(imagenUrl);
          }

          // Configurar imagen nutricional previa si existe
          if (datosSnack.imagen_nutricional) {
            const imagenNutricionalUrl =
              datosSnack.imagen_nutricional.startsWith("http")
                ? datosSnack.imagen_nutricional
                : `${API_CONFIG.BASE_URL}${datosSnack.imagen_nutricional}`;
          ...
          setMensaje("❌ Snack no encontrado");
        } else {
        ...
        setMensaje("⚠️ Error de conexión al cargar el snack");
      } finally {
        setCargando(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      if (!token) {
        setMensaje("❌ Debes iniciar sesión para editar snacks");
        setLoading(false);
        navigate("/login");
        return;
      }
      ...
      if (response.ok) {
        ...
        setMensaje("❌ No tienes permisos para editar snacks");
      } else {
        ...
        if (errorData.detail) {
          ...
          setMensaje("❌ Error al actualizar el snack. Revisa los campos.");
        }
      }
    } catch (error) {
      console.error("Error de conexión:", error);
      setMensaje("⚠️ Error de conexión al actualizar el snack");
    } finally {
      setLoading(false);
    }
  };
